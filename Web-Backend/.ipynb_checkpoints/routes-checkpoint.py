...
def get_related_by_id():
    ...
    space_name = current_app.config['NEBULA_SPACE']
    result = nebula_service.get_graph_data_by_event(id, space_name)
...
# 测试:curl -X POST "http://127.0.0.1:5001/api/search/picture" -F "file=@/data/storage/8888/xyt/work/milvus_dataset/Image/250116/watermark_image/Fake_raw_00_00_00_1.jpg" -F "topk=5"
...
def search_picture_route():
    ...
    top_k_str = request.form.get('topk', '5')
    try:
        top_k = int(top_k_str)
    except ValueError:
        return jsonify({"error": "'topk' 参数必须是整数"}), 400

    results = search_service.search_picture(filepath, top_k, current_app.config)
    ...
    return jsonify({"message": "图片上传成功", "image_base64_list": image_base64_list})
...
def search_video_route():
    ...
    results = search_service.search_video(filepath, top_k, current_app.config)
